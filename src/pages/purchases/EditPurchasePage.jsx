...
export default function EditPurchasePage() {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        cgst_percent: purchase.cgst_percent,
      ...
      // Load items
      const loadedItems = await Promise.all(purchaseDetails.items.map(async (item) => {
  ...
  const handleSave = async (print = false) => {
    ...
    try {
      ...
      const result = await window.db.invoke('purchases:update', purchaseData);
      toast.success('Purchase updated successfully');
      
      if (print) {
        // We'll implement printing in AllPurchasesPage or a dedicated print util
        toast.success('Print triggered for ' + purchaseData.invoice_no);
      }
