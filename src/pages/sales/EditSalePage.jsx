...
export default function EditSalePage() {
  ...
  const [items, setItems] = useState([
    { id: Date.now(), product_id: '', batch_id: '', batches: [], mfg_date: '', qty: '', unit: '', rate: '', amount: 0 }
  ]);
  ...
  // Adjust GST defaults based on sale type
  useEffect(() => {
    if (formData.sale_type === 'pakka') {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        cgst_percent: sale.cgst_percent,
        sgst_percent: sale.sgst_percent,
        amount_paid: sale.amount_paid,
        remarks: sale.remarks || '',
        coupon_no: sale.coupon_no || ''
      });
      setInvoiceNo(sale.invoice_no);

      // Load items
      const loadedItems = await Promise.all(saleDetails.items.map(async (item) => {
        const batches = await window.db.invoke('batches:getByProduct', item.product_id).catch(() => []);
        return {
      ...
      setItems(loadedItems.length ? loadedItems : [{ id: Date.now(), product_id: '', batch_id: '', batch_no: '', batches: [], mfg_date: '', qty: '', unit: '', rate: '', amount: 0 }]);
      ...
      const selectedParty = partiesData.find(p => p.id === sale.party_id);
  ...
  const cgstAmount = formData.sale_type === 'pakka' ? (taxableAmount * (Number(formData.cgst_percent) || 0)) / 100 : 0;
  const sgstAmount = formData.sale_type === 'pakka' ? (taxableAmount * (Number(formData.sgst_percent) || 0)) / 100 : 0;
  ...
  const handleSave = async (print = false) => {
    ...
    try {
      ...
      const saleData = {
        ...
        discount: discount,
        cgst_percent: formData.sale_type === 'pakka' ? Number(formData.cgst_percent) : 0,
        sgst_percent: formData.sale_type === 'pakka' ? Number(formData.sgst_percent) : 0,
        amount_paid: amountPaid,
        items: validItems
      };

      const result = await window.db.invoke('sales:update', saleData);
      toast.success('Sale updated successfully');
      
      if (print) {
        // We'll implement printing in AllSalesPage or a dedicated print util
        toast.success('Print triggered for ' + saleData.invoice_no);
      }
            ...
            <div className="flex space-x-4 mt-2">
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  name="sale_type"
                  value="kaccha"
                  checked={formData.sale_type === 'kaccha'}
              ...
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  ...
                  name="sale_type"
                  value="pakka"
                  checked={formData.sale_type === 'pakka'}
                  onChange={e => setFormData({ ...formData, sale_type: e.target.value })}
                  className="w-4 h-4 text-primary-600 focus:ring-primary-500"
                />
